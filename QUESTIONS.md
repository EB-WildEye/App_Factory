...
## Q1 - Which Bedrock data source id is current, «redacted:data-source-id-custom» or «redacted:data-source-id-sync» — CLOSED FOR THE FACTORY by draft ADR 0030; the production question stays open elsewhere
...
Options: (a) two data sources exist on KB «redacted:kb-id», one CUSTOM («redacted:data-source-id-custom») and one S3 («redacted:data-source-id-sync»), both live; (b) «redacted:data-source-id-sync» is stale config left over from the S3-sync era and the sync Lambda is dead code; (c) «redacted:data-source-id-custom» is stale and the scripts have not been run since.
Recommendation: (a), and confirm it in the console. `scripts/ingest_kb.py` and `scripts/kb_verify_reconstruct.py` both target «redacted:data-source-id-custom» as CUSTOM, and the sync Lambda is still deployed and wired to S3 uploads under `documents/`, so both paths look alive.
Read 2026-08-31, recorded and NOT acted on (you said this is being investigated elsewhere): `list-data-sources` on `«redacted:kb-id»` returns exactly one data source, `«redacted:data-source-id-custom»`; `get-data-source «redacted:data-source-id-sync»` returns `ResourceNotFoundException`; and `list-knowledge-bases` returns exactly one KB in `eu-west-1`, so `«redacted:data-source-id-sync»` is not a data source on some other KB either. That is evidence for option (b), not a decision. See `docs/gali-ground-truth.md` §9.4.
...
Pointer only, added 2026-09-01: the parallel investigation's report is on disk at `docs/gali_readonly_audit_2026-09-01.md`, which is **gitignored and never committed** (ADR 0039) - it is saturated with live AWS identifiers. **This entry stays open and nothing here has been acted on**, per your instruction. One finding of that report which is *not* Q1 was verified and recorded, because it contradicted an accepted ADR of mine - the prompt-limit discrepancy, now Q43.
...
Options: (a) treat the spec's values (hierarchical, parent 500, child 150, cohere.embed-multilingual-v3, 1024) as the factory standard and accept that app #1's KB may differ; (b) read the real values off KB «redacted:kb-id» in the console and make those the standard; (c) leave the KB out of the factory contract until the console read happens.
...
Resolved by (b). Read from KB `«redacted:kb-id»` on 2026-08-31: `HIERARCHICAL`, parent 500, child 150, `cohere.embed-multilingual-v3`, index dimension 1024 — **all five confirm the spec**. Recorded with the exact commands in `docs/gali-ground-truth.md` §9.
...
## Q19 - ADR 0033 (supersedes 0020): the KB vector store — REFRAMED 2026-08-31 by reading AWS
...
Recommendation, REVISED 2026-09-03 to **(b)** in ADR-0034 - one shared role with a naming-prefix wildcard, written once and never amended per app. ADR 0025's fixed bucket prefix is what made (b) available, and it gives the same shared-state guarantee as (d) with **zero** per-app writes. 0021's original recommendation was (d), with (b)'s prefix as the guard: It keeps the invariant every other step has - a provisioning step writes only resources belonging to its own app - so the IAM work folds into B1, rollback is `delete_bucket`, and the concurrent-create race on a shared policy document disappears.
...
Default taken for now: none - no middleware file exists yet. Flagged separately, not for this ADR: app #1's own API has an authentication posture that is recorded in the local-only production audit (ADR 0039) and is **not** a precedent for the factory's admin surface. A patient-facing chat endpoint and an admin endpoint holding a provisioning credential are not comparable.
...
Options: (a) copy app #1 exactly, whatever it does; (b) copy the generation and authorize the read - the server always mints the id, and the history endpoint requires proof of possession rather than knowledge of the id; (c) copy generation and format, and make the write and read paths agree on the format without adding authorization.
Recommendation: (b) for new apps; app #1 unchanged, because it is frozen. The reasoning is in `docs/gali-ground-truth.md` §10.2, stated as a requirement rather than as a reproduction: a session id is a capability, not an identity, and entropy is not the control - an id that is unguessable is still an id that travels, because it is returned to clients and systems log what they are given. So the read path authorizes; it does not accept knowledge of the id as proof of ownership.
Default taken for now: none, and nothing was redesigned - the task was to record the mechanism. **The read of app #1's actual authorization behaviour is redacted from this repository (ADR 0039)** and lives in the local-only production audit: it is a security finding about a live patient-facing clinical system, and this repository is public. Two things follow that are yours, not the factory's: whether that finding is reported to whoever owns app #1's security posture, and by whom.
...
Blocks: nothing today, and it undermines the confidence of three artefacts - ADR 0016, `docs/gali-five-parts-draft.md` (local only, ADR 0039), and the headroom arithmetic everywhere the cap is cited.
...
Surfaced by the parallel production investigation in `docs/gali_readonly_audit_2026-09-01.md` (local only, ADR 0039) and verified independently here against the service model.

## Q44 - Should the supersession rule gain a PARTIALLY SUPERSEDED status
Blocks: nothing. It is a gap in the rule you just set, found the first time the rule was applied.
The problem: the rule is written per-ADR - an accepted ADR either keeps its answer or is superseded whole. ADR 0009 has five Decision items and **only item 3 changed answer**; items 1, 2, 4 and 5 are still in force and still correct. Superseding the whole ADR would retire four good decisions to retire one wrong one, and would leave the rules-placement answer with no live home.
Options: (a) add a `PARTIALLY SUPERSEDED by ADR-00xx (item N)` form to the rule, as used on 0009 today; (b) forbid partial supersession and require the successor to restate the untouched items, so every ADR is wholly live or wholly dead; (c) treat this as a signal that an ADR carrying five separable decisions was too big, and split such ADRs on sight rather than at supersession time.
...
## Q50 - Does app #1's endpoint finding get reported, and by whom
Blocks: nothing in this repo. It is the one part of the ADR 0039 purge that removing text does not settle.
The problem: the read of app #1's endpoint authentication posture was correct and is now **out of this repository** (ADR 0039) - but it was public for ten days, and taking it down is not the same as it never having been published. Meanwhile the finding itself is unchanged: whatever the audit found is still true of the running system.
Options: (a) report it through the hospital's own channel, since the system is theirs and the freeze is theirs; (b) report it to whoever maintains the Gali backend, as an engineering matter; (c) treat the existing freeze process as the channel and raise it at the next validation review; (d) do nothing, on the basis that it is out of scope for the factory.
Recommendation: (a) or (b), and soon rather than at the next review. Not because the factory needs it - the factory needs nothing from it - but because the window in which it was public is a fact about the exposure and not about the fix, and someone who owns the system should know that before deciding what to do.
Default taken for now: none. Nothing was reported and nothing was changed in Gali. The finding is preserved verbatim in the local-only audit, which is the only copy that still exists inside this project.

## Q51 - Should the five-parts draft come back, sanitised
Blocks: Task 4's arithmetic has a second reader in that file. Nothing else.
The problem: `docs/gali-five-parts-draft.md` left git under ADR 0039 because it is Gali's clinical content rewritten - the same disclosure with different authorship, hospital phone numbers included. But it is also the factory's own design work: it is where the five parts were shown to fit inside a budget at all, measured at 4047, and three accepted ADRs cite that number.
Options: (a) leave it local-only permanently, and let the ADRs' citations stand as citations to a private measurement; (b) commit a **structural** version - the part names, the character budget per part, the arithmetic, and no clinical sentences; (c) commit it whole on the grounds that a rewrite is not production text.
Recommendation: (b). What the repository needs from that file is the shape of the budget, which is exactly the part that carries no clinical content. (c) is how this problem happened the first time: the content is not production's bytes, but it is production's meaning.
Default taken for now: (a), by omission - the file is gitignored and untracked, and nothing was rewritten. If you want (b) it is a small, contained piece of work.
