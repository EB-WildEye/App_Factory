...
Default taken for now: none. All five are listed as not found in `docs/gali-ground-truth.md` and no constant exists for any of them. A five-minute console read of `[redacted:kb-id]` either confirms the spec or invalidates five values it calls fixed for every app.
