...
| P4 | KB vector store | A Bedrock KB requires a vector store (OpenSearch Serverless, Aurora, Pinecone…). Never mentioned. Affects both create cost and B4 rollback. | `gap` — and not answerable from Gali: Gali's KB (`CHAU7BWP4S`) is a SAM **parameter**, created outside the stack, so the vector store is invisible to the repo. |
