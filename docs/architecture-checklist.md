...
| G4 | `kb_id` (`"[redacted:kb-id]"`) | `knowledgeBaseId` | casing `settled` 0008; **name still open** → 0007. Note `gali`: `[redacted:kb-id]` is not a placeholder — it is Gali's real production KB id, hard-coded at `scripts/ingest_kb.py:32`. |
